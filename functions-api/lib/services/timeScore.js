...
exports.TimeScoreService = void 0;
 ...
 * Time Score Service
 */
class TimeScoreService {
    constructor() {
        this.analyticsCollection = firebase_1.db.collection(ANALYTICS_COLLECTION);
    }
     ...
     */
    async getHistoricalData(day, hour) {
     ...
     * @return {number} Skor (0-100)
     */
    calculateHistoricalScore(records) {
        if (records.length === 0) {
     ...
     */
    async calculateCombinedScore(day, hour) {
        ...
        const combinedScore = Math.round(defaultScore * defaultWeight + historicalScore * historicalWeight);
        // Confidence level
        let confidence = "low";
        if (postCount >= 10) {
            confidence = "high";
    ...
    async getBestTimes(count = 5) {
        ...
        const recommendations = topScores.map((s) => ({
            day: types_1.DAY_NAMES_EN[s.dayOfWeek],
            dayTr: types_1.DAY_NAMES[s.dayOfWeek],
     ...
     */
    async recordPost(photoId, storyId, postedAt) {
        ...
        console.log(`[TimeScoreService] Recorded post: ${photoId} at ${types_1.DAY_NAMES[dayOfWeek]} ${hourOfDay}:00`);
        return {
            id: docRef.id,
            ...record,
        };
     ...
     * @return {Promise<void>}
     */
    async updateEngagement(analyticsId, metrics) {
    ...
    async getBestTimeForToday() {
        ...
        return {
            day: types_1.DAY_NAMES_EN[best.dayOfWeek],
            dayTr: types_1.DAY_NAMES[best.dayOfWeek],
    ...
    async getStats() {
        ...
        const postsWithEngagement = allData.filter((d) => d.engagementRate !== undefined);
        ...
        return {
            ...
            mostActiveDay: mostActiveDay
                ? types_1.DAY_NAMES[parseInt(mostActiveDay[0])]
                : "-",
            mostActiveHour: mostActiveHour ? parseInt(mostActiveHour[0]) : -1,
        };
    }
}
exports.TimeScoreService = TimeScoreService;
