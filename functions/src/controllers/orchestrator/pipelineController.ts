...
import { createHttpFunction, db, getOrchestratorConfig, errorResponse, REGION } from "./shared";
 ...
 */
export const triggerOrchestratorScheduler = createHttpFunction(async (req, res) => {
 ...
 */
export const triggerOrchestratorPipeline = createHttpFunction(async (req, res) => {
 ...
 */
export const orchestratorGenerateNow = createHttpFunction(async (req, res) => {
  if (req.method !== "POST") {
    res.status(405).json({ success: false, error: "Use POST" });
    return;
  }

  const { productType, scenarioId, themeId, aspectRatio, isRandomMode, compositionConfig, productOverrideId, preGeneratedImageBase64 } = req.body as {
  ...
  const result = await scheduler.generateNow(effectiveScenarioId, aspectRatio, productType, isRandomMode, compositionConfig, productOverrideId, preGeneratedImageBase64);
 ...
 */
export const orchestratorResendTelegram = createHttpFunction(async (req, res) => {
  if (req.method !== "POST") {
    res.status(405).json({ success: false, error: "Use POST" });
    return;
  }
  ...
  if (!slot?.pipelineResult) {
    res.status(400).json({ success: false, error: "Slot does not have pipeline result" });
    return;
  }
 ...
 */
export const validateBeforeGenerate = createHttpFunction(async (req, res) => {
  // scenarioId öncelikli, yoksa eski themeId'yi kabul et (geriye uyumluluk)
  const scenarioId = (req.query.scenarioId || req.query.themeId) as string | undefined;
  ...
  if (!scenarioId) {
    ...
    res.json({ success: true, warnings, canProceed: true });
    return;
  }
  ...
  if (!selectedScenario) {
    ...
    res.json({ success: true, warnings, canProceed: false });
    return;
  }
 ...
 */
export const assetTagAudit = createHttpFunction(async (req, res) => {
 ...
 */
export const orchestratorScheduledTrigger = onSchedule(
  {
    schedule: "*/15 * * * *",
    timeZone: TIMEZONE,
    region: REGION,
