 ...
 */
export const getMigrationStatus = createHttpFunction(async (request, response) => {
  // Tüm asset'leri say
 ...
 */
export const runCloudinaryMigration = createHttpFunction(async (request, response) => {
 ...
 * Body: { assetId: string }
 */
export const migrateSingleAsset = createHttpFunction(async (request, response) => {
  if (request.method !== "POST") {
    response.status(405).json({ success: false, error: "Use POST" });
    return;
  }
 ...
 */
export const resetMigration = createHttpFunction(async (request, response) => {
  if (request.method !== "POST") {
    response.status(405).json({ success: false, error: "Use POST" });
    return;
  }
